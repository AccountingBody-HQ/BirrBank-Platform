    ...
    <nav className="sticky top-0 z-50 bg-slate-950 border-b border-slate-800">
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between h-16">
        <Link href="/" className="flex items-center gap-2 text-white no-underline">
          ...
          <span className="font-bold text-lg tracking-tight">BirrBank</span>
        </Link>
        <div className="hidden md:flex items-center gap-1">
          {NAV.map(item => (
            <div key={item.label} className="relative"
              ...
              <Link href={item.href}
                className={"flex items-center gap-1 px-3 py-2 rounded-lg text-sm font-medium transition-colors " + (pathname.startsWith(item.href) ? 'text-white bg-white/10' : 'text-slate-400 hover:text-white hover:bg-white/5')}>
                {item.label}
                <ChevronDown size={12} className={"transition-transform " + (openDropdown === item.label ? 'rotate-180' : '')} />
              </Link>
              {openDropdown === item.label && (
                <div className="absolute top-full left-0 mt-1 w-52 bg-slate-900 border border-slate-700 rounded-xl shadow-xl overflow-hidden py-1">
                  {item.sub.map(s => (
        ...
        <div className="hidden md:flex items-center gap-3">
          <Link href="/diaspora" className="text-sm text-slate-400 hover:text-white transition-colors px-3 py-2">Diaspora</Link>
          <Link href="/sign-in" className="text-sm text-slate-400 hover:text-white transition-colors px-3 py-2">Sign in</Link>
          <Link href="/sign-up" className="text-sm font-semibold bg-green-800 hover:bg-green-700 text-white px-4 py-2 rounded-lg transition-colors">Get alerts</Link>
        </div>
        ...
        <div className="md:hidden bg-slate-950 border-t border-slate-800 px-6 py-4 space-y-1">
          {NAV.map(item => (
            <Link key={item.label} href={item.href} onClick={() => setMobileOpen(false)}
              className="block px-3 py-2.5 text-sm font-medium text-slate-400 hover:text-white hover:bg-white/5 rounded-lg transition-colors">
              {item.label}
            </Link>
          ))}
          <div className="pt-3 border-t border-slate-800 flex gap-3">
            <Link href="/sign-in" onClick={() => setMobileOpen(false)} className="flex-1 text-center text-sm text-slate-400 hover:text-white py-2">Sign in</Link>
