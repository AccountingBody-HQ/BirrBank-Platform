...
// All prices in ETB per kg. commodity_type: coffee, sesame, bean, grain

const COFFEE_PRICES = [
  { code: 'LUBPAA2', name: 'Limu Grade 2',      region: 'Limu',     price: '98,450', change: '+1,200', changePct: '+1.23', volume: '24.5t' },
  { code: 'LUBPAA3', name: 'Limu Grade 3',      region: 'Limu',     price: '92,100', change: '+850',   changePct: '+0.93', volume: '18.2t' },
  { code: 'LUBPAA5', name: 'Limu Grade 5',      region: 'Limu',     price: '84,300', change: '-400',   changePct: '-0.47', volume: '12.1t' },
  { code: 'LUBPDD5', name: 'Djimma Grade 5',    region: 'Djimma',   price: '79,600', change: '+600',   changePct: '+0.76', volume: '31.8t' },
]
...
const SUB_CATEGORIES = [
  {
    ...
    desc: "Multiple grade and origin prices from Ethiopia's largest export commodity.",
    stat: 'World 5th largest producer',
    icon: (
  ...
  },
  {
    ...
    desc: 'White and mixed sesame grade prices — Ethiopia is a top global exporter.',
    stat: 'Major export crop',
    icon: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#d97706" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <ellipse cx="12" cy="12" rx="4" ry="8"/><path d="M12 4C8 4 4 8 4 12s4 8 8 8"/><path d="M12 4c4 0 8 4 8 8s-4 8-8 8"/>
      </svg>
    ),
  },
  {
    label: 'Grain Prices',
    href: '/commodities/grains',
    desc: 'Wheat, white wheat and soybean prices from the ECX daily ticker.',
    stat: 'Food security data',
    icon: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#d97706" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M12 2a10 10 0 0 1 10 10H2A10 10 0 0 1 12 2z"/><line x1="12" y1="12" x2="12" y2="22"/>
  ...
  {
    label: 'How ECX Works',
    href: '/commodities/ecx-guide',
    desc: 'Guide to the Ethiopian Commodity Exchange — trading, grading and price discovery.',
    stat: 'Beginner guide',
    icon: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#d97706" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function CommoditiesPage() {
  return (
    <div className="min-h-screen bg-white">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-end">
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                ...
                { value: '5th',    label: 'Coffee producer rank',  sub: 'Ethiopia globally' },
                { value: '10+',    label: 'Commodity codes',       sub: 'Grades & origins' },
              ].map((s) => (
      ...
      {/* ══════════════════════════════ COFFEE PRICES ════════════════════════════ */}
      {/* NO ADS on commodity price pages — price data must be unambiguously neutral */}
      <section className="border-b border-slate-100 bg-white" style={{ padding: '96px 32px' }}>
        <div className="max-w-6xl mx-auto">
          ...
          <div className="rounded-2xl overflow-hidden border border-slate-200" style={{ boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
            <div style={{ height: 4, background: `linear-gradient(90deg, ${PILLAR}, #f59e0b)` }} />
            ...
            {COFFEE_PRICES.map((c, i) => (
              ...
              >
                ...
                >
                  <span className="font-mono font-bold text-xs rounded-lg px-2 py-1 text-center w-fit" style={{ background: PILLAR_BG, color: PILLAR }}>{c.code}</span>
                ...
                <div className="sm:hidden flex items-center gap-3" style={{ padding: '14px 16px' }}>
                  <span className="font-mono font-bold text-xs rounded-lg px-2 py-1 shrink-0" style={{ background: PILLAR_BG, color: PILLAR }}>{c.code}</span>
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-slate-800 text-sm truncate">{c.name}</p>
                    <p className="text-xs text-slate-400">{c.region} · {c.volume}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="font-mono font-black text-slate-900" style={{ fontSize: '16px' }}>{c.price}</p>
                    <p className={`font-mono font-bold text-xs ${c.change.startsWith('+') ? 'text-green-600' : 'text-red-500'}`}>{c.changePct}</p>
                  </div>
                </div>
              </div>
            ))}
              ...
              <div key={c.code} className="bg-white rounded-2xl border border-slate-200 overflow-hidden" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
                ...
                <div style={{ padding: '24px' }}>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-xs font-bold text-slate-400 rounded-full px-2 py-0.5 bg-slate-100">{c.type}</span>
                  </div>
                  <p className="font-bold text-slate-800 mb-3" style={{ fontSize: '14px' }}>{c.name}</p>
                  <p className="font-mono font-black text-slate-950 mb-1" style={{ fontSize: '24px', letterSpacing: '-0.5px', lineHeight: 1 }}>
                    {c.price}
                  </p>
