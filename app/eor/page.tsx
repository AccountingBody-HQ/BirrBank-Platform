...
import { ArrowRight, CheckCircle, XCircle, AlertCircle, Building2, Globe, Shield, Zap, Clock, DollarSign, ChevronRight } from 'lucide-react'
...
const COUNTRY_CARDS = [
  { code: 'gb', name: 'United Kingdom', flag: '🇬🇧', risk: 'Low',    speed: 'Fast',   note: 'Straightforward EOR market, strong provider choice, IR35 awareness essential.' },
  { code: 'de', name: 'Germany',        flag: '🇩🇪', risk: 'Medium', speed: 'Fast',   note: 'High employer costs (~20% SS) but mature EOR ecosystem. Works council awareness needed at scale.' },
  { code: 'fr', name: 'France',         flag: '🇫🇷', risk: 'High',   speed: 'Medium', note: 'Complex labour law and collective agreements. EOR strongly recommended over direct for first hires.' },
  { code: 'nl', name: 'Netherlands',    flag: '🇳🇱', risk: 'Low',    speed: 'Fast',   note: 'Business-friendly, predictable EOR costs, clear contractor classification rules.' },
  { code: 'es', name: 'Spain',          flag: '🇪🇸', risk: 'Medium', speed: 'Medium', note: 'High social security rates. Termination complexity makes EOR attractive for trial hires.' },
  { code: 'sg', name: 'Singapore',      flag: '🇸🇬', risk: 'Low',    speed: 'Fast',   note: 'APAC EOR hub. Simple CPF rules, English-language compliance, fast onboarding.' },
  { code: 'ae', name: 'UAE',            flag: '🇦🇪', risk: 'Low',    speed: 'Fast',   note: 'No income tax. EOR well-established in free zones. DEWS pension scheme applies.' },
  { code: 'jp', name: 'Japan',          flag: '🇯🇵', risk: 'High',   speed: 'Slow',   note: 'Complex labour law, strong employee protections, limited EOR providers. Plan carefully.' },
  { code: 'au', name: 'Australia',      flag: '🇦🇺', risk: 'Low',    speed: 'Fast',   note: 'Mature EOR market. Superannuation and Fair Work compliance well handled by providers.' },
  { code: 'ca', name: 'Canada',         flag: '🇨🇦', risk: 'Low',    speed: 'Fast',   note: 'Provincial variation adds complexity but EOR providers handle this well.' },
  { code: 'in', name: 'India',          flag: '🇮🇳', risk: 'High',   speed: 'Medium', note: 'Large EOR market but high regulatory complexity. State-level variation significant.' },
  { code: 'ie', name: 'Ireland',        flag: '🇮🇪', risk: 'Low',    speed: 'Fast',   note: 'EU gateway, English-language, low complexity. Popular EOR destination for US companies.' },
]
...
export default function EORHubPage() {
  return (
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            <div>
              ...
              <div className="space-y-5 text-slate-600 leading-relaxed">
                <p>
                  An <strong className="text-slate-900">Employer of Record (EOR)</strong> is a third-party company that
                  legally employs workers on your behalf in a country where you have no legal entity. You direct
                  the work. The EOR handles the employment contract, payroll, taxes, benefits, and compliance.
                </p>
                <p>
                  The EOR becomes the legal employer on paper. Your company retains full operational control —
                  you set the job scope, working hours, and performance expectations. The EOR takes on the
                  compliance liability.
                </p>
                <p>
                  EOR is not the same as a staffing agency. The workers are dedicated entirely to your
                  organisation. They are not shared or placed on short contracts by default. EOR is a permanent
                  hiring solution for companies that have not yet set up a local entity.
                </p>
              </div>
              <div className="mt-8 p-5 bg-blue-50 border border-blue-200 rounded-xl">
                ...
                <p className="text-blue-700 text-sm leading-relaxed">
                  With direct employment, your company is legally responsible for every payroll error,
                  missed filing, and wrongful termination claim. With EOR, that liability transfers to
                  the provider — who specialises in exactly this.
                </p>
              </div>
            </div>
            <div>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-24">
          <div className="mb-12">
            ...
            <p className="text-slate-500 mt-4 max-w-2xl leading-relaxed">
              Three models for employing people internationally. Each has its place — the right choice
              depends on headcount, timeline, and how long you plan to operate in the market.
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
            ...
            {EOR_VS_TABLE.map((row, i) => (
              <div key={row.factor} className={`grid grid-cols-1 lg:grid-cols-[2fr_1.5fr_1.5fr_1.5fr] gap-2 lg:gap-0 px-6 py-4 ${i > 0 ? 'border-t border-slate-100' : ''} hover:bg-blue-50/40 transition-colors`}>
                ...
                <span className="text-sm text-blue-700 font-semibold lg:flex lg:items-center">
                  <span className="lg:hidden text-xs text-slate-400 font-normal mr-1">EOR: </span>{row.eor}
                </span>
                <span className="text-sm text-slate-600 lg:flex lg:items-center">
                  <span className="lg:hidden text-xs text-slate-400 font-normal mr-1">Direct: </span>{row.direct}
                </span>
                <span className="text-sm text-slate-600 lg:flex lg:items-center">
                  <span className="lg:hidden text-xs text-slate-400 font-normal mr-1">PEO: </span>{row.peo}
                </span>
              </div>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-24">
          <div className="mb-12">
            ...
            <div className="flex items-end justify-between gap-6">
              <h2 className="font-serif text-4xl font-bold text-slate-900 tracking-tight leading-tight">
                Estimate your EOR cost<br />before you commit.
              </h2>
              <p className="hidden lg:block text-slate-500 max-w-md leading-relaxed">
                Select a country, enter your headcount and average salary. We calculate the estimated
                total employer cost — including social security, taxes, and the EOR provider markup.
              </p>
            </div>
          </div>
          <EORCostEstimator />
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-24">
          <div className="mb-12">
            ...
            <div className="flex items-end justify-between">
              <h2 className="font-serif text-4xl font-bold text-slate-900 tracking-tight leading-tight">
                EOR complexity by country.
              </h2>
              <Link href="/countries/" className="flex items-center gap-2 text-blue-600 hover:text-blue-700 font-semibold text-sm">
                All countries <ArrowRight size={15} />
              </Link>
            </div>
            <p className="text-slate-500 mt-4 max-w-2xl leading-relaxed">
              Risk rating, typical onboarding speed, and key compliance notes for major EOR markets.
            </p>
          </div>

          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {COUNTRY_CARDS.map(c => (
              <Link key={c.code} href={`/eor/${c.code}/`}
                ...
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className={`text-xs font-bold px-2.5 py-1 rounded-full border ${riskColour[c.risk]}`}>
                    {c.risk} risk
                  </span>
                </div>
                ...
                <div className="mt-5 flex items-center justify-between">
                  <span className="text-xs text-slate-400">
                    Speed: <span className={`font-bold ${speedColour[c.speed]}`}>{c.speed}</span>
                  </span>
                  <span className="flex items-center gap-1 text-blue-600 text-xs font-bold uppercase tracking-wide opacity-0 group-hover:opacity-100 transition-opacity">
                    Full guide <ChevronRight size={11} />
                  </span>
                </div>
              </Link>
            ))}
      ...
      <section className="relative overflow-hidden" style={{backgroundColor: '#0d1f3c'}}>
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-8 py-24 text-center">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            <Link href="/countries/" className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white font-bold px-7 py-4 rounded-xl transition-colors">
              Browse all countries <ArrowRight size={15} />
            </Link>
            <Link href="/payroll-tools/" className="inline-flex items-center gap-2 bg-white/10 hover:bg-white/15 border border-white/20 text-white font-semibold px-7 py-4 rounded-xl transition-colors">
              Payroll calculators
            </Link>
          </div>
        </div>
