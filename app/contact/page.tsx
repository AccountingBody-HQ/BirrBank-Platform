        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-8 pt-24 pb-20">
          <div className="max-w-3xl">
            ...
            <h1 className="font-serif text-4xl lg:text-6xl font-bold text-white leading-[1.08] mb-8" style={{ letterSpacing:'-0.025em' }}>
              Get in touch with the BirrBank team.
            </h1>
            <p className="text-lg text-slate-400 leading-relaxed max-w-2xl">
              General enquiries, data correction reports, or partnership questions — we read every message and respond within two business days.
            </p>
          </div>
        </div>
      </section>
            ...
            <div className="lg:col-span-1">
              ...
              <div className="space-y-6">
                <div className="bg-white border border-slate-200 rounded-2xl p-6">
                  ...
                  <p className="text-slate-500 text-sm leading-relaxed">Questions about the platform, data coverage, or partnership opportunities.</p>
                </div>
                <div className="bg-white border border-slate-200 rounded-2xl p-6">
                  <div className="bg-blue-600 text-white w-10 h-10 rounded-xl flex items-center justify-center mb-4">
                    <AlertCircle size={18} />
                  ...
                  <p className="text-slate-500 text-sm leading-relaxed">Found an error in a savings rate, FX rate, or other data point? Report it and we will verify and update it promptly.</p>
                </div>
                <div className="bg-white border border-slate-200 rounded-2xl p-6">
                  <div className="bg-blue-600 text-white w-10 h-10 rounded-xl flex items-center justify-center mb-4">
                    <Mail size={18} />
            ...
            <div className="lg:col-span-2">
              <div className="flex gap-2 mb-8 bg-white border border-slate-200 rounded-xl p-1.5 w-fit">
                <button onClick={() => setFormType('general')}
                  className={`px-5 py-2.5 rounded-lg text-sm font-semibold transition-all ${formType === 'general' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}>
                  General enquiry
                </button>
                <button onClick={() => setFormType('data')}
                  className={`px-5 py-2.5 rounded-lg text-sm font-semibold transition-all ${formType === 'data' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}>
                  Data correction
                </button>
              </div>
              ...
              {status === 'success' ? (
                <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-10 text-center">
                  ...
                  <button onClick={() => setStatus('idle')} className="mt-6 text-blue-600 font-semibold text-sm hover:text-blue-700 transition-colors">
                ...
                <form onSubmit={handleSubmit} className="bg-white border border-slate-200 rounded-2xl p-8 space-y-5">
                  <div className="grid sm:grid-cols-2 gap-5">
                    <div>
                      ...
                      <input name="full_name" required placeholder="Abebe Girma"
                        className="w-full px-4 py-3 bg-white border border-slate-200 focus:border-blue-500 rounded-xl outline-none transition-colors text-slate-900 placeholder:text-slate-400 text-sm" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Email address *</label>
                      <input name="email" type="email" required placeholder="abebe@example.com"
                        className="w-full px-4 py-3 bg-white border border-slate-200 focus:border-blue-500 rounded-xl outline-none transition-colors text-slate-900 placeholder:text-slate-400 text-sm" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Organisation</label>
                    <input name="organisation" placeholder="Company or institution (optional)"
                      className="w-full px-4 py-3 bg-white border border-slate-200 focus:border-blue-500 rounded-xl outline-none transition-colors text-slate-900 placeholder:text-slate-400 text-sm" />
                  </div>
                  {formType === 'data' && (
                    <>
                      <div className="grid sm:grid-cols-2 gap-5">
                        <div>
                          <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Data section *</label>
                          <select name="section" required defaultValue=""
                            className="w-full px-4 py-3 bg-white border border-slate-200 focus:border-blue-500 rounded-xl outline-none transition-colors text-slate-900 text-sm">
                            <option value="" disabled>Select a section</option>
                            <option>Savings rates</option>
                            <option>FX rates</option>
                            <option>Insurance premiums</option>
                            <option>ESX equity prices</option>
                            <option>IPO pipeline</option>
                            <option>T-bill yields</option>
                            <option>ECX commodity prices</option>
                            <option>Remittance fees</option>
                            <option>Institution data</option>
                            <option>Other</option>
                          </select>
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Data field *</label>
                          <input name="data_field" required placeholder="e.g. CBE 12-month savings rate"
                            className="w-full px-4 py-3 bg-white border border-slate-200 focus:border-blue-500 rounded-xl outline-none transition-colors text-slate-900 placeholder:text-slate-400 text-sm" />
                        </div>
                      </div>
                      <div className="grid sm:grid-cols-2 gap-5">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
                      {formType === 'data' ? 'Additional notes' : 'Message *'}
                    </label>
                    <textarea name="message" required={formType === 'general'} rows={5}
                      placeholder={formType === 'data' ? 'Any additional context...' : 'How can we help?'}
                      className="w-full px-4 py-3 bg-white border border-slate-200 focus:border-blue-500 rounded-xl outline-none transition-colors text-slate-900 placeholder:text-slate-400 text-sm resize-none" />
                  </div>
                  {status === 'error' && (
                  ...
                  <button type="submit" disabled={status === 'sending'}
                    ...
                    {status === 'sending' ? 'Sending...' : formType === 'data' ? 'Submit correction' : 'Send message'}
