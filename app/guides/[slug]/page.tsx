...
const portableComponents = {
  block: {
    normal: ({ children }: any) => (
      <p className="text-slate-600 my-4" style={{ fontSize: '16px', lineHeight: 1.85 }}>{children}</p>
    ),
    h2: ({ children }: any) => (
      <h2 className="font-serif font-bold text-slate-950 mt-10 mb-4" style={{ fontSize: 'clamp(20px, 2.5vw, 26px)', letterSpacing: '-0.8px' }}>{children}</h2>
    ),
    h3: ({ children }: any) => (
      <h3 className="font-bold text-slate-900 mt-8 mb-3" style={{ fontSize: '18px' }}>{children}</h3>
    ),
  },
  list: {
    bullet: ({ children }: any) => <ul className="my-4 space-y-2">{children}</ul>,
    number: ({ children }: any) => <ol className="my-4 space-y-2 list-decimal list-inside">{children}</ol>,
  },
  listItem: {
    bullet: ({ children }: any) => (
      <li className="flex items-start gap-3 text-slate-600" style={{ fontSize: '15px', lineHeight: 1.75 }}>
    ...
    number: ({ children }: any) => (
      <li className="text-slate-600" style={{ fontSize: '15px', lineHeight: 1.75 }}>{children}</li>
    ),
  },
  marks: {
    strong: ({ children }: any) => <strong className="font-bold text-slate-800">{children}</strong>,
    em: ({ children }: any) => <em className="italic">{children}</em>,
    code: ({ children }: any) => <code className="bg-slate-100 rounded px-1 text-sm font-mono">{children}</code>,
  },
}
      ...
      <section className="relative bg-white overflow-hidden border-b border-slate-100">
        ...
        <div className="relative max-w-4xl mx-auto px-8 pt-20 pb-14">
          ...
          <div className="flex flex-wrap items-center gap-4 text-xs text-slate-400">
            {fmtDate(guide.publishedAt) && <span>Published {fmtDate(guide.publishedAt)}</span>}
            <span>·</span>
