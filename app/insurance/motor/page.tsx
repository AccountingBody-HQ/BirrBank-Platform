...
export default async function MotorInsurancePage() {
  ...
  const { data: productsData } = await supabase.schema('birrbank').from('insurance_products').select('*, institutions(name, slug)').eq('product_type', 'motor').eq('is_current', true).order('annual_premium_pct', { ascending: true })
  const { count: totalCount } = await supabase.schema('birrbank').from('insurance_products').select('count', { count: 'exact', head: true }).eq('product_type', 'motor').eq('is_current', true)
    ...
    <div className="min-h-screen bg-white">
      <section className="relative bg-white overflow-hidden border-b border-slate-100">
        ...
        <div className="relative max-w-6xl mx-auto px-8 pt-20 pb-14">
          ...
          <p className="text-xs font-black uppercase tracking-widest mb-4" style={{ color: PILLAR }}>Insurance · Motor</p>
          <h1 className="font-serif font-bold mb-5 text-slate-950" style={{ fontSize: 'clamp(36px, 4.5vw, 54px)', letterSpacing: '-1.8px', lineHeight: 1.08 }}>
            Motor insurance in Ethiopia —<br /><span style={{ color: PILLAR }}>all providers compared.</span>
          </h1>
          <p className="text-slate-600 mb-8" style={{ fontSize: '16px', lineHeight: '1.8', maxWidth: '520px' }}>
            Third-party and comprehensive motor insurance from every NBE-licensed insurer.
            The NBE sets a minimum premium schedule — compare who charges what above the minimum.
          </p>
          <div className="flex flex-wrap gap-6">
            {[
              { icon: <ClockIcon />, label: (totalCount ?? 0) + ' products compared' },
              { icon: <ClockIcon />, label: 'NBE minimum premium schedule' },
      ...
      <section className="bg-white" style={{ padding: '64px 32px 96px' }}>
        <div className="max-w-6xl mx-auto">
          <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-8">Showing {totalCount ?? 0} products · Sorted by premium (low to high)</p>
      ...
      <section className="border-b border-slate-100" style={{ background: '#f9fafb', padding: '96px 32px' }}>
        <div className="max-w-6xl mx-auto">
          <div className="mb-10">
            ...
            <h2 className="font-serif font-bold text-slate-950" style={{ fontSize: 'clamp(26px, 3vw, 36px)', letterSpacing: '-1.2px', lineHeight: 1.15 }}>What to look for when choosing motor insurance.</h2>
          </div>
      ...
      <section className="bg-white" style={{ padding: '96px 32px' }}>
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <h2 className="font-serif font-bold text-slate-950 mb-5" style={{ fontSize: 'clamp(30px, 3.5vw, 42px)', letterSpacing: '-1.5px', lineHeight: 1.1 }}>Motor premium changes,<br /><span style={{ color: PILLAR }}>direct to your inbox.</span></h2>
            <p className="text-slate-500 mb-8" style={{ fontSize: '15px', lineHeight: 1.85 }}>Get notified when insurers update their motor insurance premiums or launch new products.</p>
            <ul className="space-y-3 mb-8">
              {['Motor insurance premium changes across all providers','New comprehensive product launches','NBE minimum premium schedule changes','Claims guide and process updates'].map((item) => (
