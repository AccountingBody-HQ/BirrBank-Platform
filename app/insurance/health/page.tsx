...
export default async function HealthInsurancePage() {
  ...
  const { data: productsData } = await supabase.schema('birrbank').from('insurance_products').select('*, institutions(name, slug)').eq('product_type', 'health').eq('is_current', true).order('premium_from_etb', { ascending: true })
  const { count: totalCount } = await supabase.schema('birrbank').from('insurance_products').select('count', { count: 'exact', head: true }).eq('product_type', 'health').eq('is_current', true)
  const products = productsData ?? []

  const GUIDE = [
    { step: '01', title: 'Inpatient vs outpatient cover', body: 'Inpatient cover pays for hospitalisation costs including surgery, room and board, and nursing. Outpatient cover pays for GP visits, specialist consultations, diagnostics and pharmacy. Most policies offer both but with different limits.' },
    { step: '02', title: 'Key exclusions to check', body: 'Pre-existing conditions are commonly excluded in the first year. Cosmetic procedures, experimental treatments and self-inflicted conditions are typically excluded permanently. Always read the exclusions list before purchasing.' },
    { step: '03', title: 'Individual vs group health plans', body: 'Group plans through employers are typically cheaper per person due to risk pooling. Individual plans cost more but give you continuity if you change employer. For SMEs, group health insurance can be a significant staff retention tool.' },
  ]
    ...
    <div className="min-h-screen bg-white">
      <section className="relative bg-white overflow-hidden border-b border-slate-100">
        ...
        <div className="relative max-w-6xl mx-auto px-8 pt-20 pb-14">
          ...
          <p className="text-xs font-black uppercase tracking-widest mb-4" style={{ color: PILLAR }}>Insurance · Health Insurance</p>
          <h1 className="font-serif font-bold mb-5 text-slate-950" style={{ fontSize: 'clamp(36px, 4.5vw, 54px)', letterSpacing: '-1.8px', lineHeight: 1.08 }}>
            Health insurance in Ethiopia —<br /><span style={{ color: PILLAR }}>individual and group plans compared.</span>
          </h1>
          <p className="text-slate-600 mb-8" style={{ fontSize: '16px', lineHeight: '1.8', maxWidth: '520px' }}>Individual and group health insurance from all NBE-licensed providers. Compare inpatient cover, outpatient limits, maternity benefits and annual premiums.</p>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <h2 className="font-serif font-bold text-slate-950 mb-5" style={{ fontSize: 'clamp(30px, 3.5vw, 42px)', letterSpacing: '-1.5px', lineHeight: 1.1 }}>
              Health Insurance changes,<br /><span style={{ color: PILLAR }}>direct to your inbox.</span>
            </h2>
            <p className="text-slate-500 mb-8" style={{ fontSize: '15px', lineHeight: 1.85 }}>Get notified when insurers update their health insurance premiums or launch new products.</p>
            <ul className="space-y-3 mb-8">
              {['Health Insurance premium changes across all providers','New product launches and coverage updates','NBE minimum premium schedule changes','Claims guide and process updates'].map((item) => (
