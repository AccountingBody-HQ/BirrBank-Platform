...
const ANON_KEY = "birrbank_ai_anon_count";
...
function SignUpPrompt() {
  return (
    <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6 text-center my-2">
      <h3 className="text-slate-900 font-bold text-lg mb-1">Enjoying BirrBank AI?</h3>
      <p className="text-slate-500 text-sm mb-4">Create a free account for 10 questions per month.</p>
      <div className="flex gap-2 justify-center">
        <a href="/sign-up" className="text-white font-semibold px-5 py-2.5 rounded-xl text-sm transition-colors" style={{background:"#1D4ED8"}}>Create free account</a>
        <a href="/sign-in" className="bg-white border border-slate-200 text-slate-700 font-semibold px-5 py-2.5 rounded-xl text-sm transition-colors">Sign in</a>
      </div>
    </div>
  );
}
...
function UpgradePrompt() {
  return (
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm my-2">
      ...
      <div className="p-6">
        <h3 className="text-slate-900 font-bold text-lg mb-1">BirrBank Intelligence</h3>
        <p className="text-slate-500 text-sm mb-4">You have used your 10 free questions this month. The Intelligence tier is coming soon.</p>
        <div className="grid grid-cols-2 gap-2 mb-4">
          {["Unlimited AI questions","Saved comparisons","Rate alerts","Premium market data","ECX price history","ESX portfolio tools","NBE directive alerts","Diaspora FX tracker"].map(f => (
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-center text-sm text-slate-500">
          Intelligence tier — coming soon. <Link href="/contact" className="text-blue-600 font-semibold hover:underline">Join the waitlist</Link>
        </div>
      </div>
    </div>
  );
...
const SUGGESTIONS = [
  "What are the best savings rates in Ethiopia right now?",
  "Compare USD buying rates across Ethiopian banks",
  "How do I send money to Ethiopia from the UK?",
  "Which banks offer Sharia-compliant accounts?",
];

export default function AiChatClient({ userId, monthlyUsage, freeAnonLimit, freeUserLimit }: Props) {
  ...
  function checkLimit(): "anon"|"free"|null {
    ...
    if (userId && usageCount >= freeUserLimit) return "free";
    return null;
  }
  ...
  async function sendMessage(text?: string) {
    ...
    try {
      ...
      if (userId && usageCount + 1 >= freeUserLimit) setShowLimit("free");
    } catch {
      setThinking(false);
      ...
      <div className="border-b border-slate-200 bg-white shrink-0 shadow-sm">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="text-slate-900 font-bold text-sm">BirrBank AI</div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-4 pb-8 min-h-0 overflow-y-auto">
          <div className="w-full max-w-2xl">
            <div className="text-center mb-6">
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-md" style={{background:"#1D4ED8"}}>
                <svg className="w-7 h-7 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z"/>
                </svg>
              </div>
              <h1 className="text-slate-900 font-bold text-2xl mb-1">BirrBank AI</h1>
              <p className="text-slate-500 text-sm">Specialist AI for Ethiopian banking, FX rates, insurance, ESX markets, and NBE-regulated institutions</p>
            </div>
            <div className="grid grid-cols-2 gap-2.5 mb-5">
              {SUGGESTIONS.map(s => (
                <button key={s} onClick={() => sendMessage(s)}
                  ...
                  <span className="font-bold mr-1" style={{color:"#1D4ED8"}}>+</span>{s}
                </button>
              ))}
            </div>
            <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-4">
              <div className="flex gap-2 items-end">
                <textarea
                  ...
                  placeholder="Ask about Ethiopian banks, FX rates, insurance, ESX markets..."
                  rows={1}
                ...
                <button onClick={() => sendMessage()} disabled={!input.trim() || loading}
                  className="disabled:opacity-30 text-white rounded-xl px-4 py-2.5 transition-colors shrink-0" style={{background:"#1D4ED8"}}>
            ...
            <p className="text-slate-400 text-xs mt-2.5 text-center">Covers Ethiopian financial services only. Always confirm with a qualified financial professional.</p>
          </div>
        </div>
      )}
        ...
        <>
            ...
            <div className="max-w-3xl mx-auto px-4 py-6 flex flex-col gap-6">
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                  {msg.role === "user" ? (
                    <div className="max-w-[78%] text-white rounded-2xl rounded-tr-sm px-5 py-3.5 text-sm leading-relaxed shadow-sm" style={{background:"#1D4ED8"}}>
                      {msg.content}
                    </div>
                  ) : (
                    <div className="flex-1 group">
                      <div className="flex items-center gap-2 mb-2">
                        <AIIcon />
                        <span className="text-slate-400 text-xs font-semibold uppercase tracking-wide">BirrBank AI</span>
                      </div>
          ...
          <div className="border-t border-slate-200 bg-white shrink-0 shadow-lg">
            <div className="max-w-3xl mx-auto px-4 py-3">
              <div className="bg-white border border-slate-200 rounded-2xl shadow-sm px-4 py-3">
                <div className="flex gap-2 items-end">
                  <textarea
                    ...
                    onChange={e => setInput(e.target.value)}
                    onKeyDown={handleKeyDown}
                    placeholder={showLimit ? "Create an account to continue..." : "Ask about Ethiopian banks, FX rates, insurance, ESX markets..."}
                  ...
                  <button onClick={() => sendMessage()} disabled={!input.trim() || loading || !!showLimit}
                    className="disabled:opacity-30 disabled:cursor-not-allowed text-white rounded-xl px-4 py-2.5 transition-colors shrink-0" style={{background:"#1D4ED8"}}>
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5"/>
                    </svg>
                  </button>
              ...
              <p className="text-slate-400 text-xs mt-2 text-center">Covers Ethiopian financial services only. Always confirm with a qualified financial professional.</p>
            </div>
          </div>
        </>
      )}
